...
- **Template**
  - `tmpl/default.php`:
    - Loads assets via Joomla WebAsset Manager (`mod_qlbigslide.script`, `mod_qlbigslide.style`).
    ...
    - Passes slider settings to JavaScript using `Document::addScriptOptions('mod_qlbigslide.config', [...])` and the wrapper methods:
      - `getAutoplayMs()`
      - `getBoxAlign()`
...
- **JavaScript**
  - `media/js/script.js`:
    - Reads options from `Joomla.getOptions('mod_qlbigslide.config')` and exposes them as `window.SLIDER_CONFIG`:
      - `autoplayMs`
      - `boxAlign`
      - `displayNavigationPrevNext`
      - `displayNavigationDots`
...
1. Install or copy the module into your Joomla instance so that it is available as `mod_qlbigslide`.
2. In the Joomla Administrator, go to **Extensions → Modules** and create or edit a module of type **Qlbigslide**.
...
- Settings are stored as module params in `mod_qlbigslide.xml`.
